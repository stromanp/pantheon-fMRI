...
for grouptype in grouptype_list:
	for region_name in region_name_list:
		...
		figoutputname = r'D:\Howie_FM2_Brain_Data\compare_stim_rest_{}{}_Sept2024.svg'.format(grouptype1,region_name)
		figoutputname_strat = r'D:\Howie_FM2_Brain_Data\compare_stim_rest_{}{}_painsens_strat_Sept2024.svg'.format(grouptype1,region_name)
		...
		if datatype1 == 'brain':
			...
			if grouptype2 == 'HC':
				covariates_file1 = r'D:\Howie_FM2_Brain_Data\HCstim_02323203331202_V5_results_covariates.npy'
			else:
				covariates_file1 = r'D:\Howie_FM2_Brain_Data\FMstim_02323203331202_V5_results_covariates.npy'
		...
		if datatype2 == 'brain':
			...
			if grouptype2 == 'HC':
				covariates_file2 = r'D:\Howie_FM2_Brain_Data\HCstim_02323203331202_V5_results_covariates.npy'
			else:
				covariates_file2 = r'D:\Howie_FM2_Brain_Data\FMstim_02323203331202_V5_results_covariates.npy'
		...
		tbaseline = list(range(30,int(timeperiod1[0]),2)) + list(range(int(timeperiod1[1]), int(timeperiod2[0]),2)) + list(range(int(timeperiod2[1]),270,2))
...
def look_at_initial_rise(covariateofinterest):
	# covariate_list = ['painrating','temperature','wpi','COMPASS','SDS','FIQR_SIQR','STAI_Y_1','STAI_Y_2','PCS_Total']
	...
	cindex = covariates['GRPcharacteristicslist'].index(covariateofinterest)
	covariate_values = covariates['GRPcharacteristicsvalues'][cindex,:]
...
#--------second set of data for comparison-------------------------
	...
	cindex2 = covariates2['GRPcharacteristicslist'].index(covariateofinterest)
	covariate_values2 = covariates2['GRPcharacteristicsvalues'][cindex2,:]
...
# ---------end of second set of data-------------------------------
	...
	# correlation between pain ratings and initial intensity
	cov = copy.deepcopy(covariate_values)
	cov2 = copy.deepcopy(covariate_values2)
	...
	xx = list(range(len(cov)))
	xx2 = list(range(len(cov2)))

	c = [x for x in range(len(cov)) if not np.isfinite(cov[x])]
	if len(c) > 0: cov[c] = 0.0
	c = [x for x in range(len(cov2)) if not np.isfinite(cov2[x])]
	if len(c) > 0: cov2[c] = 0.0

	if covariateofinterest.lower()[:3] == 'sta':
		xx = np.where(cov < 60)[0]   # for STA1 and STA2
		xx2 = np.where(cov2 < 60)[0]

	if covariateofinterest.lower() == 'compass':
		xx = np.where(cov < 100)[0]   # for compass
		xx2 = np.where(cov2 < 100)[0]
	...
	# [2, 7, 9, 12, 24, 25, 44, 47, 83, 89]
	region_fullname = []
	ancova_p = np.zeros((len(nlist), 10))
	for ncount, nn in enumerate(nlist):
		cc = np.where(nn >= nclusterend)[0]
		rnum = cc[-1]
		cnum = nn - nclusterend[rnum]
		region_fullname += ['{} {}'.format(rnamelist[rnum],cnum)]
		...
		ancova_p[ncount, :] = np.array([p_MeoG, p_MeoC, p_intGC, df, group_F, group_p, cov_F, cov_p, inter_F, inter_p])
	...
	dftext = ['{:.4f}'.format(ancova_p[x,3]) for x in range(len(nlist))]
	groupFtext = ['{:.4f}'.format(ancova_p[x,4]) for x in range(len(nlist))]
	groupptext = ['{:.4f}'.format(ancova_p[x,5]) for x in range(len(nlist))]
	covFtext = ['{:.4f}'.format(ancova_p[x,6]) for x in range(len(nlist))]
	covptext = ['{:.4f}'.format(ancova_p[x,7]) for x in range(len(nlist))]
	interFtext = ['{:.4f}'.format(ancova_p[x,8]) for x in range(len(nlist))]
	interptext = ['{:.4f}'.format(ancova_p[x,9]) for x in range(len(nlist))]
	...
	textoutputs = {'region': np.array(region_fullname), 'group F':np.array(groupFtexts), 'group p':np.array(groupptexts),
				   'covariate F':np.array(covFtexts), 'covariate p':np.array(covptexts),
				   'interaction F': np.array(interFtexts), 'interaction p': np.array(interptexts),
				   'p': np.array(psorted)}

	# p, f = os.path.split(SAPMresultsname)
	df = pd.DataFrame(textoutputs)
	xlname = os.path.join(datadir, 'initialrise_vs_covariates.xlsx')
	if os.path.isfile(xlname):
		mode = 'a'
	else:
		mode = 'w'
	with pd.ExcelWriter(xlname, engine = 'openpyxl', mode=mode) as writer:
		df.to_excel(writer, sheet_name=covariateofinterest)
	outputname = xlname
	print('wrote {} values to {} sheet in {}'.format(len(c),covariateofinterest,xlname))
