...
def gradients_in_intrinsic2(Sinput, Soutput, intrinsic1, intrinsic2, beta_int1, Minput, Moutput, dvali):
    ...
    # fit, S_full = network_fit(S_full, Minput, Moutput)   # use the quick method for estimates
    # fit, S_full = network_approach_method(S_full, Minput, Moutput, nregions)
    # fit, S_full = network_descent_L1(S_full, Minput, Moutput, nregions)
    ...
    for nn in range(nI):
        ...
        dssq_dI[nn] = (ssqdp - ssqd) / dvali

    return dssq_dI, ssqd


def gradients_in_beta1(Sinput, Soutput, intrinsic1, intrinsic2, beta_int1, Minput, Moutput, dval):
    ...
    err = Sin_full[:nregions, :] - fit[:nregions, :]
    cost = np.sum(np.abs(dint)) + np.sum(np.abs(intrinsic2)) + np.sum(np.abs(betavals))
    ssqd = np.sum(err ** 2) + Lweight * cost  # L1 regularization
...
def gradients_for_betavals(Sinput, Soutput, intrinsic1, intrinsic2, beta_int1, Minput, Moutput, betavals, ctarget, csource, dval):
    nregions, tsize_full = np.shape(Sinput)
    ...
    # fit, Soutput_temp = network_fit(Soutput_full, Minput, Moutput)    # use the quick method for estimates
    # fit, Soutput_temp = network_approach_method(Soutput_full, Minput, Moutput, nregions)
    # fit, Soutput_temp = network_descent_L1(Soutput_full, Minput, Moutput, nregions)
    ...
    cost = np.sum(np.abs(beta_int1)) + np.sum(np.abs(intrinsic2)) + np.sum(np.abs(betavals))
    ssqd = np.sum(err ** 2) + Lweight * cost  # L1 regularization
    ...
    for nn in range(nbeta):
        ...
        err = Sinput_full[:nregions, :] - fit[:nregions, :]
        cost = np.sum(np.abs(beta_int1)) + np.sum(np.abs(intrinsic2)) + np.sum(np.abs(b))
        ssqdp = np.sum(err ** 2) + Lweight * cost  # L1 regularization
...
for nperson in range(NP):
    ...
    # intrinsic2 = np.random.randn(tsize_total)    # initialize unknown intrinsic with small random values
    # intrinsic2 = center_tc_by_run(intrinsic2, nruns)   # keep all tc values at a mean of zero
    intrinsic2 = np.zeros(tsize_total)    # initialize unknown intrinsic with small random values

    intrinsic2 = np.mean(Sinput,axis=0)
    intrinsic2 = center_tc_by_run(intrinsic2, nruns)   # keep all tc values at a mean of zero
    ...
    while bigiter < bigitermax:
        ...
        # fit, Soutput_full = network_descent_L1(Soutput_full, Minput, Moutput, nregions)   # update Soutput estimate more accuarately
        # Soutput = Soutput_full[:nregions,:]
        # fit, Soutput_temp = network_fit(Soutput_full, Minput, Moutput)    # use the quick method for estimating starting ssqd
        ...
        while alphai > alpha_limit  and iter < nitermax  and converging:
            ...
            ssqd_record += [ssqd]
            dssq_dbeta1, ssqd = gradients_in_beta1(Sinput, Soutput, intrinsic1, intrinsic2, beta_int1, Minput, Moutput, dval)
            ssqd_record += [ssqd]

            # apply the changes
            intrinsic2 -= alphai * dssq_dI
            beta_int1 -= alphab * dssq_dbeta1

            intrinsic2_record.append({'i2':intrinsic2})
        ...
        while alpha > alpha_limit  and iter < nitermax and converging:
            ...
            dssq_db, ssqd = gradients_for_betavals(Sinput, Soutput, intrinsic1, intrinsic2, beta_int1, Minput, Moutput,
                                             betavals, ctarget, csource, dval)
            ssqd_record += [ssqd]
            ...
            Moutput[ctarget,csource] = betavals
            # fit, Soutput_temp = network_fit(Soutput_full, Minput, Moutput)   # use the quick method
            # fit, Soutput_temp = network_approach_method(Soutput_full, Minput, Moutput, nregions)
            # fit, Soutput_temp = network_descent_L1(Soutput_full, Minput, Moutput, nregions)
            fit, Soutput_temp = network_eigenvalue_method(Soutput_full, Minput, Moutput, nregions)
    ...
    # fit, Soutput_full = network_fit(Soutput_full, Minput, Moutput)
    # fit, Soutput_full = network_descent_L1(Soutput_full, Minput, Moutput, nregions)   # use the more accurate method for updating
    # fit, Soutput_full = network_approach_method(Soutput_full, Minput, Moutput, nregions)
    fit, Soutput_full = network_eigenvalue_method(Soutput_full, Minput, Moutput, nregions)
    ...
    plt.plot(range(tsize_total),Sinput_full[regionnum1,:],'-ob')
    plt.plot(range(tsize_total),fit[regionnum1,:],'-xr')

    plt.close(window2)
    fig = plt.figure(window2, figsize=(12.5, 3.5), dpi=100)
    plt.plot(range(tsize_total),Sinput_full[regionnum2,:],'-ob')
    plt.plot(range(tsize_total),fit[regionnum2,:],'-xr')

    # plot intrinsic2
    plt.close(20)
    fig = plt.figure(20, figsize=(12.5, 3.5), dpi=100)
    plt.plot(range(tsize_total),intrinsic2,'-og')
    ...
    Z1 = np.arctanh(R1[0, 1]) * np.sqrt(197)
    ...
    Z2 = np.arctanh(R2[0, 1]) * np.sqrt(197)
...
if check_results:
    ...
    for nperson in person_list:
        ...
        # ax1 = fig.add_subplot(3,2,4, sharey = True)
        ax1b.plot(Sinput[region1, :], '-ob')
        ax1b.plot(fit[region1, :], '-xr')
        ax1b.set_title('target {}'.format(rnamelist[region1]))
        ax2b.plot(Sinput[region2, :], '-ob')
        ax2b.plot(fit[region2, :], '-xr')
        ax2b.set_title('target {}'.format(rnamelist[region2]))
        ax3b.plot(Sinput[region3, :], '-ob')
        ax3b.plot(fit[region3, :], '-xr')
