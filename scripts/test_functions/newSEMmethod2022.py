...
for nperson in range(NP):    # select one person (for testing)
    ...
    fit0 = fit[0,:]
    S0 = Sinput_full[0,:]
    Scalc0 = Minput[0,:] @ Soutput_full
    check = Minput @ Moutput
    Scheck0 = check[0,:] @ Soutput_full

    plt.close(27)
    fig = plt.figure(27)
    plt.plot(range(tsize_full),S0,'-xb')
    plt.plot(range(tsize_full),fit0,'-r')

    plt.close(28)
    fig = plt.figure(28)
    plt.plot(range(tsize_full),S0,'-xb')
    plt.plot(range(tsize_full),Scalc0,'-r')

    plt.close(29)
    fig = plt.figure(29)
    plt.plot(range(tsize_full),S0,'-xb')
    plt.plot(range(tsize_full),Scheck0,'-r')

    Soutput_full_actual = Moutput @ Soutput_full
    Scheck = Soutput_full_actual[5,:] + Soutput_full_actual[6,:] + Soutput_full_actual[10,:]

    S0 = Sinput_full[0,:]
    nruns = nruns_per_person[nperson]
    S0b = np.mean(np.reshape(S0,(nruns,tsize)),axis = 0)
    Scheckb = np.mean(np.reshape(Scheck,(nruns,tsize)),axis = 0)
    S5 = np.mean(np.reshape(Soutput_full_actual[5,:],(nruns,tsize)),axis = 0)
    S6 = np.mean(np.reshape(Soutput_full_actual[6,:],(nruns,tsize)),axis = 0)
    S10 = np.mean(np.reshape(Soutput_full_actual[10,:],(nruns,tsize)),axis = 0)

    plt.close(30)
    fig = plt.figure(30)
    plt.subplot(4,1,1)
    plt.plot(range(tsize),S0b,'-xb')
    plt.plot(range(tsize),Scheckb,'-r')
    plt.subplot(4,1,2)
    plt.plot(range(tsize),S5,'-xb')
    plt.subplot(4,1,3)
    plt.plot(range(tsize),S6,'-xb')
    plt.subplot(4,1,4)
    plt.plot(range(tsize),S10,'-xb')
