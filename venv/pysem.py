...
#-----------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------
def pysem_network_search(cluster_properties, region_properties, networkmodel, timepoints, epoch, savedirectory, savenamelabel):
    print('running pysem_network search ...')
    network, ncluster_list, sem_region_list = pyclustering.load_network_model(networkmodel)
    nclusterlist = np.array([ncluster_list[i]['nclusters'] for i in range(len(ncluster_list))])
    ntargets = len(network)
    nregions = len(cluster_properties)
    ...
    # determine SEM fit obtained with one set of component loading values ...
    # PCparams = {'components': component_data, 'loadings': original_loadings}
    # construct the timeseries data for a set of PC loadings...
    Sinput = []
